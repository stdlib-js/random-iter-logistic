...
function iterator( mu, s, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0q16v', mu ) );
	}
	if ( !isPositive( s ) ) {
		throw new TypeError( format( '0q172', s ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0q12V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0q12t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
