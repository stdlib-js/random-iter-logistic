...
function iterator( mu, s, options ) {
	...
	if ( !isNumber( mu ) || isnan( mu ) ) {
		throw new TypeError( format( '0q16v,NI', mu ) );
	}
	if ( !isPositive( s ) ) {
		throw new TypeError( format( '0q172,Go', s ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0q12V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0q12t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
